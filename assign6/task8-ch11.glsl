...
void main() {
    ...
    float vfov = 30.0; // should be 20

    vec3 lookfrom = vec3(-2.0, 2.0, 1.0); //good
    vec3 lookat = (vec3(0.0, 0.0, -1.0)); // good
    vec3 vup = vec3(0.0, 1.0, 0.0); // good 

    float focal_length = length((lookfrom - lookat)); // good

    vec3 w = normalize(lookfrom - lookat); // good 
    vec3 u = normalize(cross(vup, w)); // good
    vec3 v = cross(w, u); // good

    float theta = radians(vfov); // good
    float h = tan(theta / 2.0); // good
    float viewport_height = 2.0 * h * focal_length; // good
    float viewport_width = viewport_height * aspect; // good

    vec3 viewport_u = viewport_width * u; // good
    vec3 viewport_v = viewport_height * v; // good

    vec3 pixel_delta_u = viewport_u / aspect; // good
    vec3 pixel_delta_v = viewport_v / aspect; // good

    vec3 viewport_lower_left = lookfrom - (focal_length * w) - (viewport_u / 2.0) - (viewport_v / 2.0);  // good
    ...
    material s2 = material(vec3(1.0), 3, 1.5); //left sphere
    ...
    // material s0 = material(vec3(0.0, 0.0, 1.0), 1, 0.0); //ground sphere
    // material s1 = material(vec3(1.0, 0.0, 0.0), 1, 0.0); //center sphere

    // Sphere spheres[2];
    // spheres[0] = Sphere(vec3(cos(PI/4.0), 0.0, -1.0), cos(PI/4.0), s0); //center sphere
    // spheres[1] = Sphere(vec3(-cos(PI/4.0), 0.0, -1.0), cos(PI/4.0), s1); //center sphere
    ...
    for(int i = 0; i < 100; i++){
        ...
        vec3 ray_direction =  normalize(cam.lower_left_corner + uv.x * cam.horizontal + uv.y * cam.vertical - cam.origin);
