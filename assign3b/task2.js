...
// ################ Edit your code below
        vec3 lightDirection = normalize(LightPosition - (Model * vec4(Position, 1.0)).xyz);
        vec3 transformedNormal = normalize((NormalMatrix * vec4(Normal, 0.0)).xyz);

        vec3 lightIntesity = LightPosition - (Model * vec4(Position, 1.0)).xyz;
        float distance = dot(lightIntesity, lightIntesity);

        Color = ka + kd * (LightIntensity/distance) * max(dot(transformedNormal, lightDirection), 0.0);
...
var Task2 = function (gl) {
  this.pitch = 0;
  this.yaw = 0;
  this.sphereMesh = new ShadedTriangleMesh(
    gl,
    SpherePositions,
    SphereNormals,
    SphereTriIndices,
    LambertVertexSource,
    LambertFragmentSource
  );
  this.cubeMesh = new ShadedTriangleMesh(
    gl,
    CubePositions,
    CubeNormals,
    CubeIndices,
...
Task2.prototype.render = function (gl, w, h) {
  ...
  var view = Matrix.rotate(-this.yaw, 0, 1, 0)
    .multiply(Matrix.rotate(-this.pitch, 1, 0, 0))
    .multiply(Matrix.translate(0, 0, 5))
    .inverse();
  var rotation = Matrix.rotate(Date.now() / 25, 0, 1, 0);
  var cubeModel = Matrix.translate(-1.8, 0, 0).multiply(rotation);
  var sphereModel = Matrix.translate(1.8, 0, 0)
    .multiply(rotation)
    .multiply(Matrix.scale(1.2, 1.2, 1.2));
